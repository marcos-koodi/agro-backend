...
module.exports = {
    ...
    async listaDocsEtpDocsCli(req, res) {
        const token = req.headers['x-access-token'];
        const { id_cliente_servico } = req.body;
        if (!token) return res.status(401).json({ auth: false, message: 'No token provided.' });
        ...
        jwt.verify(token, process.env.SECRET, async function (err, decoded) {
            ...
            try {
                ...
                const res_list_docs_etapa = await knex.select(
                    'td.id AS id_doc', 'td.tipo AS tipo', 'td.nome AS nome_doc')
                    .from('documentos_etapa AS de').innerJoin('tipo_documento AS td', 'td.id', 'de.id_documento')
                    .where('de.id_etapa', 1).where('td.setor', 'Pessoal').orderBy('de.id');
                ...
                async function getListDocs(id, id_cliente_servico) {
                    ...
                    const listComStatus = await knex.select('docCli.id', 'docCli.id_cliente', 'docCli.tipo_doc', 'docCli.status', 'tpDoc.id AS tipo_doc', 'tpDoc.nome', 'docCli.doc_anexo AS nome_file', 'tpDoc.tipo', 'tpDoc.setor')
                        .from('documentos_cliente AS docCli').innerJoin('tipo_documento AS tpDoc ', 'tpDoc.id', 'docCli.tipo_doc')
                        .where('id_cliente', id).where('tpDoc.setor', 'Pessoal');

                    // console.log("1 listComStatus", listComStatus);

                    resId_cliente_servico_etapa = await knex.select('id').from('cliente_servico_etapa')
                        .where('id_cliente_servico', id_cliente_servico)
                        .where('etapa', 1);

                    console.log("2 resId_cliente_servico_etapa");

                    documentos_cliente_servico_etapa = await knex.select('docsEtp.status', 'docsEtp.tipo_doc', 'tpDoc.nome', 'docsEtp.doc_anexo AS nome_file', 'tpDoc.setor', 'tpDoc.tipo', 'docsEtp.id_cliente_servico_etapa', 'docsEtp.id')
                        .from('documentos_cliente_servico_etapa AS docsEtp')
                        .innerJoin('tipo_documento AS tpDoc', 'tpDoc.id', 'docsEtp.tipo_doc')
                        .where('docsEtp.id_cliente', id)
                        .where('docsEtp.id_cliente_servico_etapa', resId_cliente_servico_etapa[0]['id'])
                    ...
                    console.log("3 documentos_cliente_servico_etapa");
                    //  ^retorna -->    {
                    //         "status": 2,
                    //         "tipo_doc": "7",
                    //         "nome": "Certidão de Matricula do Bem",
                    ...
                    /**
                     ...
                     const listSemStatus = await knex.select('*').from('tipo_documento')
                     .whereNotIn('id',id_doc)
                     .whereNotIn('id',id_docsEtp)
                     .where('setor','Pessoal');
                     ...
                     resDataHerdeiros[index]['lista_docPessoal'] = tp_lista; */
                    resDataHerdeiros[index]['lista_docPessoal'] = listComStatus;
                ...
                var docs_cli_etp = [{ 'clientes': [], 'docs_etapa': [] }];

                docs_cli_etp[0]['clientes'].push(resDataHerdeiros);

                docs_cli_etp[0]['docs_etapa'].push(res_list_docs_etapa);

                return res.json({ data: docs_cli_etp, status: 200, message: "Carregando os documentos do cliente" });
            } catch (error) {
                return res.json({ data: error, status: 400, message: "Não foi possivel carregar os clientes e documentos peossoais" });
            };
        });
    ...
    async listDocEtapaPropriedade(req, res) {
        const token = req.headers['x-access-token'];
        const { id_cliente_servico } = req.body;
        if (!token) return res.status(401).json({ auth: false, message: 'No token provided.' });
        ...
        jwt.verify(token, process.env.SECRET, async function (err, decoded) {
            ...
            try {
                ...
                // status, tipo_doc, nome, setor, 
                const listDocPropriedade = await knex.select('docsEtp.status', 'docsEtp.tipo_doc', 'tpDoc.nome', 'docsEtp.doc_anexo AS nome_file', 'tpDoc.setor', 'tpDoc.tipo', 'docsEtp.id_cliente_servico_etapa', 'docsEtp.id')
                    .from('documentos_cliente_servico_etapa AS docsEtp')
                    .innerJoin('tipo_documento AS tpDoc', 'tpDoc.id', 'docsEtp.tipo_doc')
                    .where('docsEtp.id_cliente_servico_etapa', resId_cliente_servico_etapa[0]['id']).where('tpDoc.setor', 'Propriedade');
                ...
                const res_list_docs_etapa = await knex.select(
                    'td.id AS id_doc', 'td.tipo AS tipo', 'td.nome AS nome_doc')
                    .from('documentos_etapa AS de').innerJoin('tipo_documento AS td', 'td.id', 'de.id_documento')
                    .where('de.id_etapa', 1).where('td.setor', 'Propriedade').orderBy('de.id');
                ...
                docsEtapaProp =
                {
                    'docsEtapaProp': res_list_docs_etapa,
                    'docsPropriedade': tp_listaProp
                };

                return res.json({ data: docsEtapaProp, status: 200, message: "Carregando os documentos de propriedade" });
            } catch (error) {
                return res.json({ data: error, status: 400, message: "Não foi possivel carregar os documentos de propriedade" });
            };
        });
        ...
        jwt.verify(token, process.env.SECRET, async function (err, decoded) {
            ...
            try {
                var response = await knex.select('cs.id as id_cliente_servico', 'cs.id_servico AS id_servico', 'cs.herdeiro_1',
                    'cs.herdeiro_2', 'exp.nome AS nome_expansionista', 'cs.id_projetista', 'cs.id_expansionista', 'adm.nome AS nome_projetista', 's.nome AS nome_servico', 's.icone', 's.descricao AS descricao_servico', 'cs.data_cad', 'cse.status')
                    .from('cliente_servico AS cs')
                    .innerJoin('cliente_servico_etapa as cse', 'cs.id', 'cse.id_cliente_servico')
                    .innerJoin('servicos AS s', 'cs.id_servico', 's.id')
                    .leftJoin('user_admin AS adm', 'cs.id_projetista', 'adm.id')
                    .leftJoin('expansionista AS exp', 'cs.id_expansionista', 'exp.id')
                    //'cse.etapa'
                    // .where(function () {
                ...
                if (response.length > 0) {
                    for (let i = 0; i < response.length; i++) {
                        ...
                        if (!(response[i].nome_expansionista)) {
                            response[i].nome_expansionista = "Não definido";
                        }
                        if (!(response[i].nome_projetista)) {
                            response[i].nome_projetista = "Não definido";
                        }
    ...
    async isCadServico(req, res) {
        ...
        jwt.verify(token, process.env.SECRET, async function (err, decoded) {
            if (err) return res.status(500).json({ auth: false, message: 'Failed to authenticate token.' });
            try {
                var response = await knex.select('cs.id as id_cliente_servico', 'cs.id_servico AS id_servico',
                    'cs.herdeiro_1', 'cs.herdeiro_2', 'cs.data_cad')
                    .from('cliente_servico AS cs')
                ...
                if (response.length > 0) {
                    return res.json({ data: response, status: 200, message: "Já possui um serviço em andamento." });
                } else {
                    return res.json({ data: response, status: 540, message: "Não possui um serviço em andamento." });
                }
            } catch (error) {
                return res.json({ data: error, status: 400, message: "Não foi possivel verificar se há serviço em andamento" });
            }
        });
    },

    async cadDocCliente(req, res) {
        const { id_cliente, tipo_doc, documento, tipo, id_cliente_servico, etapa, nome_file } = req.body;
        ...
        jwt.verify(token, process.env.SECRET, async function (err, decoded) {
            ...
            let data_cad = new Date();
            if (tipo == 1) {
                const response = await knex('documentos_cliente').insert({ id_cliente, tipo_doc, documento, data_cad, doc_anexo: nome_file });
                if (response.length > 0) {
                ...
                const response = await knex('documentos_cliente_servico_etapa').insert({ id_cliente, tipo_doc, documento, id_cliente_servico_etapa: id_cliente_servico_etapa[0]['id'], data_cad, doc_anexo: nome_file });
                if (response.length > 0) {
                    console.log("Doc Cadastrado.");
                    return res.json({ message: 'documentos_cliente cadastrado com sucesso.', status: 200, data: response })
                } else {
